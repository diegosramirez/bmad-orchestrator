...
class EpicStoryBreakdown(BaseModel):
    ...
    When Discovery+Architecture describe both a client app and a server/API, prefer two stories
    (backend + frontend under contract); see _stories_breakdown_create prompt.
    """
...
def make_create_story_tasks_node(
    ...
    *,
    ...
    def _stories_breakdown_create(state: OrchestratorState) -> dict[str, Any]:
        """Create stories under the epic; default to BE+FE pair when epic spans client and API."""
        ...
        user_msg = (
            ...
            "AND (2) a server/API, persistence, or backend that serves data to that client, "
            "then default to **TWO** stories unless the epic text explicitly demands a single "
            "vertical slice or a tiny scope that truly fits one story:\n"
            "  - Story A (backend): REST/API, data model, persistence if any, business rules, "
            "consistent JSON/error shapes; deliverable is testable without any SPA (e.g. API "
            "tests, curl, OpenAPI). No UI work in this story.\n"
            "  - Story B (frontend): UI/UX, components, client routing/state, a11y; use mocks, "
            "MSW, or fixtures matching the agreed contract until the real API is integrated. No "
            "new server route or DB migration work in this story.\n"
            "  - Name the contract (endpoints, payloads, errors) in one story or the epic; use "
            "dependencies to note parallel work with mocks vs blocked on merged API.\n"
            ...
            "epic spans client + API — split as above unless the epic explicitly says otherwise.\n"
            "- Backend and frontend stories must each be concrete and completable: backend owns "
            "DB/API/server; frontend uses mock-backed HTTP aligned to the contract when needed.\n"
            ...
            "Angular interceptors, MSW in the browser, or any mobile UI toolkit. Backend "
            "acceptance is verifiable against the running API (or contract doc) only.\n"
            "  - In the **frontend** story: do NOT describe implementing server HTTP handlers, "
            "new REST controller code, ORM/DB migrations, server-side auth middleware, or "
            "deploying the API service as part of this story. The frontend may define TypeScript "
            "types and client services that **call** the API or mocks; it must not own server "
            "endpoint implementation.\n"
            "  - Optional shared artifact: the **API contract** (paths, schemas, errors) may be "
            "referenced in both stories as text or OpenAPI, but implementation of routes "
            "belongs only in the backend story; implementation of UI and client mocks belongs "
            "only in the frontend story.\n"
