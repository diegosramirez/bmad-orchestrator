    ...
    *,
    ...
    def dev_story_fix_loop(state: OrchestratorState) -> dict[str, Any]:
        ...
        prompt = (
            ...
            f"2. Do NOT modify build config files (angular.json, tsconfig.json, "
            f"vitest.config.ts, jest.config.ts, webpack.config.js, etc.) unless "
            f"the review issue specifically requires it.\n"
            f"3. If you change code behavior, also update the corresponding "
            f"test assertions to match. Tests and code must stay in sync.\n"
