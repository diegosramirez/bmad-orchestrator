    ...
    *,
    ...
    def e2e_automation(state: OrchestratorState) -> dict[str, Any]:
        ...
        prompt = (
            ...
            f"2. **Set up Playwright if not already installed:**\n"
            f"   a. Run `npm install -D @playwright/test` to add it as a dev dependency.\n"
            f"   b. Run `npx playwright install chromium` to install the browser binary.\n"
            f"   c. Add a `\"test:e2e\": \"playwright test\"` script to package.json "
            f"(if one doesn't already exist for E2E).\n"
            f"3. If playwright.config.ts doesn't exist, create it with a webServer "
            f"option to auto-start the dev server.\n"
            f"4. Write Playwright E2E tests that validate each acceptance criterion "
            f"from the user's perspective.\n"
            f"5. Use semantic locators: getByRole, getByLabel, getByText — avoid "
            f"CSS selectors.\n"
            f"6. Write ONE test file per tool call to avoid output token limits.\n"
