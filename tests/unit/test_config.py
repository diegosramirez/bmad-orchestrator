...
def test_settings_model_copy_overrides() -> None:
    ...
    s2 = s.model_copy(update={"dry_run": True, "model_name": "claude-3-haiku-20240307"})
    assert s2.dry_run is True
    assert s2.model_name == "claude-3-haiku-20240307"
...
def test_settings_agent_models_user_override_wins() -> None:
    s = Settings(
        ...
        jira_project_key="P",
        github_repo="o/r",
        agent_models={"developer": "claude-3-haiku-20240307", "pm": "claude-3-haiku-20240307"},
    )
    # User override takes precedence over built-in default
    assert s.agent_models["developer"] == "claude-3-haiku-20240307"
    assert s.agent_models["pm"] == "claude-3-haiku-20240307"
